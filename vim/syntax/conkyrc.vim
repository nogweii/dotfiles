...
syn region ConkyrcComment start=/^\s*#/ end=/$/ contains=@Spell

syn keyword ConkyrcSetting
       \ alignment background on_bottom border_margin border_width
       \ cpu_avg_samples default_color default_shade_color default_outline_color
       \ double_buffer draw_borders draw_shades draw_outline font gap_x gap_y
       \ no_buffers mail_spool maximum_width minimum_size mldonkey_hostname
       \ mldonkey_port mldonkey_login mldonkey_password mpd_host mpd_port
       \ mpd_password net_avg_samples override_utf8_locale own_window
       \ own_window_transparent own_window_colour pad_percents stippled_borders
       \ total_run_times update_interval uppercase use_spacer use_xft
       \ out_to_console out_to_x border_inner_margin border_outer_margin
       \ if_up_strictness short_units xftfont
...
syn region ConkyrcText start=/^TEXT$/ end=/\%$/ contains=ConkyrcVar,@Spell
