...
function! s:ReadDir(lpn,dir) " <<<
	...
	"exec ("j" . (repeat ("l", strlen(foldprt) )) )

	let ln = getline(line(".")+1)
	let treeprt = substitute (ln, '[^-| `].*', "", "")
	exec ("normal! j" . (repeat ("l", strlen(treeprt) )) )
...
"" IsRoot() - return true/false if VTreeExplorer is at the root directory
function! s:IsRoot() " <<<
	let cwd = getcwd()
	if has("unix") == 0 
		let cwd = substitute (cwd, '\\', '/', "g")
		if exists("g:treeExplLocalRoot")
			let localRoot = substitute (g:treeExplLocalRoot, '\\', '/', "g")
			let is_root = (cwd == localRoot) ? 1 : 0
		else
			let is_root = (cwd =~ '^[A-Z]:/$') ? 1 : 0
		end
	else
		if exists("g:treeExplLocalRoot")
			let is_root = (cwd == g:treeExplLocalRoot) ? 1 : 0
		else
			let is_root = (cwd == "/") ? 1 : 0
		end
	endif
