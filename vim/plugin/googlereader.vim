...
function! s:ShowEntry()
  ...
  setlocal buftype=nofile bufhidden=hide noswapfile wrap ft= nonumber modifiable nospell
  silent! %d _
  redraw!
...
function! s:ShowEntryInBrowser()
  ...
  if exists("g:googlereader_browser")
    call system(printf(g:googlereader_browser, escape(b:url, '#')))
  elseif has('win32')
...
function! s:ShowEntries(opt)
  ...
  setlocal buftype=nofile bufhidden=hide noswapfile nowrap ft= nonumber modifiable cursorline nospell
  silent! %d _
  redraw!
