...
function! s:CmdLine_Twitter(initstr, inreplyto)
    ...
    call inputsave()
    let mesg = input("Your tweet: ", a:initstr)
    call inputrestore()
...
function! s:twitter_win(wintype)
	...
	if a:wintype != "userinfo"
	    " Automatically update the timeline every 30 secs, just let the cursor sit still
	    setlocal updatetime=30000
	    au! CursorHold <buffer> :call <SID>RefreshTimeline()
	    ...
	    " Vim emulation:
	    " * i - New tweet
	    " * o - Reply to the current line's tweet
	    " * gf - Go to the user page for the cword
	    " * gF - Get the user's timeline
	    " * r - retweet
	    " * dd - delete your tweet
	    " * dm - DM the user
	    nnoremap <buffer> <silent>  i :call <SID>CmdLine_Twitter('', 0)<cr>
	    nnoremap <buffer> <silent>  o :call <SID>Quick_Reply()<cr>
	    nnoremap <buffer> <silent>  r :call <SID>Retweet()<cr>
	    nnoremap <buffer> <silent> dm :call <SID>Quick_DM()<cr>
	...
	" Get user information (or their timeline)
	nnoremap <buffer> <silent> gf :call <SID>get_user_info(<SID>get_user_cword())<cr>
	nnoremap <buffer> <silent> gF :call <SID>get_timeline("user", <SID>get_user_cword(), 1)<cr>
    endif
...
" Return the cWORD with @ and : stripped (leaving just the user name)
function! s:get_user_cword()
