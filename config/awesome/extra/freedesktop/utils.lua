...
function parse_desktop_files(arg)
    local programs = {}
    local files = io.popen('find '.. arg.dir ..' -maxdepth 1 -name "*.desktop" 2>/dev/null'):lines()
...
function parse_dirs_and_files(arg)
    local files = {}
    local paths = io.popen('find '..arg.dir..' -maxdepth 1 -type d 2>/dev/null'):lines()
    ...
    local paths = io.popen('find '..arg.dir..' -maxdepth 1 -type f 2>/dev/null'):lines()
    for path in paths do
